...
static WORD DayOfMon[12] = { 31,28,31,30,31,30,31,31,30,31,30,31 };
...
void CalcNextDateTime(SYSTEMTIME& stTime)
{
	WORD wYear = stTime.wYear;
	WORD wMon = stTime.wMonth;
	WORD wDay = stTime.wDay;

	//判断月底
	if (wDay == DayOfMon[wMon] || (wMon==2 && wDay==29))
	{
		if (wMon==2 && wDay==28)
		{
			if (((wYear % 4 == 0) && (wYear % 100 != 0)) || (wYear % 400 == 0))
			{
				stTime.wDay += 1;	//闰年的2月底
				return;
			}
		}
