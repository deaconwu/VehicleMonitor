...
bool CSmtp::CreateConn()
{
	...
	SOCKADDR_IN addrSrv;
	HOSTENT* pHostent = gethostbyname(domain.c_str());  //得到有关于域名的信息
...
{/*格式化要发送的内容*/
	...
	char* pTitleBuff = base64Encode(title.c_str(), strlen(title.c_str()));
	std::string strTitle = pTitleBuff;
	email += "=?gb2312?B?";
	email += strTitle;
	email += "?=";
	email += "\r\n";
